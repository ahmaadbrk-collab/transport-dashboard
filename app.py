...
def dashboard():
    ...
    <head>
        ...
        <style>
            ...
            .nav a {{ background: #8e44ad; color: white; padding: 10px 25px; border-radius: 5px; text-decoration: none; font-size: 15px; font-weight: bold; }}
            .nav a:hover {{ background: #7d3c98; }}
    ...
    <body>
        ...
        <div class="nav">
            <a href="/predictions">🔮 التوقعات والتنبيهات</a>
        </div>

        <div class="filters">
            <form method="get" style="display:flex; gap:15px; align-items:center; flex-wrap:wrap;">
                <div><label>السائق: </label><select name="driver">{driver_options}</select></div>
                <div><label>من مدينة: </label><select name="from_city">{from_options}</select></div>
                <div><label>إلى مدينة: </label><select name="to_city">{to_options}</select></div>
                <div><label>من تاريخ: </label><input type="date" name="date_from" value="{date_from}"></div>
                <div><label>إلى تاريخ: </label><input type="date" name="date_to" value="{date_to}"></div>
...
# ==============================
# صفحة التوقعات والتنبيهات
# ==============================
@app.route('/predictions')
def predictions():

    # 1. توقع إيرادات الشهر القادم
    monthly_revenue = df.groupby('MonthLabel')['Revenue'].sum().reset_index()
    monthly_revenue = monthly_revenue.sort_values('MonthLabel')
    avg_revenue = monthly_revenue['Revenue'].mean()
    last_month = monthly_revenue.iloc[-1]['MonthLabel'] if not monthly_revenue.empty else 'غير متوفر'
    last_month_revenue = monthly_revenue.iloc[-1]['Revenue'] if not monthly_revenue.empty else 0
    trend = 'ارتفاع' if last_month_revenue > avg_revenue else 'انخفاض'
    trend_icon = '📈' if last_month_revenue > avg_revenue else '📉'
    trend_color = '#2ecc71' if last_month_revenue > avg_revenue else '#e74c3c'

    # 2. توقع أفضل وجهة الشهر القادم (بناءً على آخر 3 أشهر)
    last_3_months = monthly_revenue.tail(3)['MonthLabel'].tolist()
    recent_df = df[df['MonthLabel'].isin(last_3_months)]
    top_dest_recent = recent_df.groupby('FromPos')['Profit'].sum().nlargest(1)
    best_future_dest = top_dest_recent.index[0] if not top_dest_recent.empty else 'غير متوفر'
    best_future_dest_val = top_dest_recent.iloc[0] if not top_dest_recent.empty else 0

    # 3. تنبيهات السائقين الذين انخفض أداؤهم
    driver_all = df.groupby('DriverName')['Profit'].mean().reset_index()
    driver_all.columns = ['DriverName', 'AvgAll']

    last_month_df = df[df['MonthLabel'] == last_month]
    driver_last = last_month_df.groupby('DriverName')['Profit'].mean().reset_index()
    driver_last.columns = ['DriverName', 'AvgLast']

    merged = driver_all.merge(driver_last, on='DriverName', how='inner')
    merged['Drop'] = merged['AvgAll'] - merged['AvgLast']
    merged['DropPct'] = (merged['Drop'] / merged['AvgAll'] * 100).round(1)
    declined = merged[merged['DropPct'] > 20].sort_values('DropPct', ascending=False)

    # بناء تنبيهات السائقين
    if declined.empty:
        alerts_html = '<div class="alert-ok">✅ لا يوجد سائقون انخفض أداؤهم بشكل ملحوظ</div>'
    else:
        alerts_html = ''
        for _, row in declined.iterrows():
            ...
            <div class="alert-card">
                ...
                <div class="alert-text">
                    ...
                    انخفض متوسط ربحه بنسبة <span style="color:#e74c3c; font-weight:bold;">{row["DropPct"]}%</span>
                    مقارنة بمتوسطه العام
                </div>
            </div>
            '''
    ...
    <html dir="rtl">
    <head>
        <title>التوقعات والتنبيهات</title>
        <style>
            body {{ font-family: Arial; margin: 20px; background: #f0f2f5; }}
            h1 {{ color: #2c3e50; text-align: center; margin-bottom: 10px; }}
            .nav {{ text-align: center; margin-bottom: 25px; }}
            .nav a {{ background: #3498db; color: white; padding: 10px 25px; border-radius: 5px; text-decoration: none; font-size: 15px; font-weight: bold; }}
            .section-title {{ font-size: 20px; font-weight: bold; color: #2c3e50; margin: 25px 0 15px 0; border-right: 4px solid #3498db; padding-right: 10px; }}
            .cards {{ display: flex; gap: 15px; flex-wrap: wrap; margin-bottom: 20px; }}
            .card {{ background: white; border-radius: 10px; padding: 25px; flex: 1; min-width: 220px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }}
            .card-icon {{ font-size: 40px; margin-bottom: 10px; }}
            .card-title {{ color: #7f8c8d; font-size: 14px; margin-bottom: 5px; }}
            .card-val {{ font-size: 26px; font-weight: bold; color: #3498db; }}
            .card-sub {{ font-size: 13px; color: #7f8c8d; margin-top: 5px; }}
            .alert-card {{ background: white; border-radius: 10px; padding: 15px 20px; margin-bottom: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); display: flex; align-items: center; gap: 15px; border-right: 4px solid #e74c3c; }}
            .alert-ok {{ background: white; border-radius: 10px; padding: 15px 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); border-right: 4px solid #2ecc71; font-size: 16px; color: #2ecc71; font-weight: bold; }}
            .alert-icon {{ font-size: 28px; }}
            .alert-text {{ font-size: 15px; color: #2c3e50; }}
        </style>
    </head>
    <body>
        <h1>🔮 التوقعات والتنبيهات</h1>

        <div class="nav">
            <a href="/">← العودة للداشبورد</a>
        </div>

        <div class="section-title">توقع إيرادات الشهر القادم</div>
        <div class="cards">
            <div class="card">
                <div class="card-icon">💰</div>
                <div class="card-title">متوسط الإيرادات الشهرية</div>
                <div class="card-val">{avg_revenue:,.0f}</div>
                <div class="card-sub">التوقع للشهر القادم بناءً على المتوسط</div>
            </div>
            <div class="card">
                <div class="card-icon">{trend_icon}</div>
                <div class="card-title">آخر شهر مقارنة بالمتوسط</div>
                <div class="card-val" style="color:{trend_color};">{trend}</div>
                <div class="card-sub">آخر شهر: {last_month_revenue:,.0f} — المتوسط: {avg_revenue:,.0f}</div>
            </div>
        </div>

        <div class="section-title">توقع أفضل وجهة الشهر القادم</div>
        <div class="cards">
            <div class="card">
                <div class="card-icon">🗺️</div>
                <div class="card-title">الوجهة المتوقعة الأعلى ربحاً</div>
                <div class="card-val">{best_future_dest}</div>
                <div class="card-sub">بناءً على أداء آخر 3 أشهر — ربح: {best_future_dest_val:,.0f}</div>
            </div>
        </div>

        <div class="section-title">⚠️ تنبيهات السائقين</div>
        {alerts_html}
